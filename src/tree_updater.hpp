...
namespace strom
{
  ...
  // reset function to re-initialize any temporary variables
  ...
  /**
   * \brief Propose new state for TreeUpdater
   *
   * In the TreeUpdater, we randomly choose an internal node x that is not the root and has parent y that is also not the root.
   * We then randomly choose two children of x, a and b, and a node below b (which might be y itself).
   * We then choose a point on the path from a to b and slide either the node above the chosen point (i.e. x) or the node below the chosen point (i.e. y) to the chosen point.
   * If the chosen point is below the middle node (i.e. x), the move involves a topology change.
   */
  inline void TreeUpdater::proposeNewState()
  {
    ...
    // Now choose a different chilf of x (or the parent) to be node _b
    ...
    // Symmetric Mode: Hastings ratio = 1
    _log_hastings_ratio = 0.0;
   ...
   *
   * This function is called if the last proposal is rejected. It reverses the
