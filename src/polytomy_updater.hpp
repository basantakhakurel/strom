...
namespace strom
{
  ...
  /// Destructor for PolytomyUpdater class.
  /// This function handles the cleanup when a PolytomyUpdater object is destroyed.
  inline PolytomyUpdater::~PolytomyUpdater()
  {
    // std::cout << "Deleting a PolytomyUpdater" << std::endl;
  }

  /// Resets the state of the PolytomyUpdater.
  ///
  /// This function clears any stored information about the tree and
  /// resets all member variables to their default values, so that the
  /// PolytomyUpdater is ready to be used again.
  ...
  /// Calculates the prior probability of the current tree topology and edge lengths.
  ...
  /**
   * Proposes a new tree topology by either adding an edge to create a split
   * in a polytomy or deleting an edge to create a polytomy. The proposal
   * invalidates all transition matrices and partials.
   *
  ...
  inline void PolytomyUpdater::proposeNewState()
  {
    ...
    if (tree->isRooted())
    {
      // remember: root node is internal because it had a left child
  ...
  /**
   * @brief Proposes an add-edge move to split a polytomy.
   *
   * This function attempts to resolve a polytomy by introducing a new internal node
   * and an edge, effectively splitting the polytomy into two parts. A new node `v` is
   * created and connected to the given node `u`, which represents the polytomy.
   * A subset of the spokes (children nodes) is chosen randomly to connect to the new node `v`.
   *
   * The function calculates the probability of each possible partitioning of the chosen
   * polytomy and randomly selects a valid partition. The partitioning ensures that both
   * `u` and `v` retain valid tree structures. Edge lengths are adjusted proportionally
   * to maintain the tree's total length.
   *
   * If a revert is necessary, the changes are tracked using `_orig_par` and `_orig_lchild`.
   *
   * @param u Pointer to the node representing the polytomy to be split.
   */
  ...
  /**
   * \brief Delete the edge associated with `v' to create a polytomy (or a bigger polytomy if `v->par' was already a polytomy).
  ...
  /**
   * @brief Reverts the current state of the tree to the previous state.
   *
   * This function is called when a proposed tree is rejected. It reverses the
   * effects of the last proposal, so that the tree is in the same state as it
   * was before the proposal was made. This is done by swapping nodes if
   * necessary, and then setting the edge lengths of the nodes to their
   * original values.
   */
  ...
  /**
   * @brief Refreshes the internal list of polytomies.
   *
   * This function clears the internal list of polytomies and then
   * iterates over the tree, adding each node that has more than two
