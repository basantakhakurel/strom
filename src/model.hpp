...
namespace strom
{
  ...
  {
    // Creates a summary such as following and returns as a string:
    //
    ...
    // Map for storing string that will contain the information for each row
    ...
    // Ensure that the subset relative rates are fized if there is only one
    ...
    for (unsigned i = 0; i < _num_subsets; i++)
    {
      ...
      std::transform(freqs.begin(), freqs.end(), freqs_as_strings.begin(), [](double freq)
                     { return boost::str(boost::format("%g") % freq); });
    ...
    for (unsigned i = 0; i < _num_subsets; i++)
    {
      if (_subset_datatypes[i].isNucleotide())
      {
        ...
        std::transform(xchg.begin(), xchg.end(), xchg_as_strings.begin(), [](double x)
                       { return boost::str(boost::format("%g") % x); });
  ...
  // function to calulate and return the sum of the products of subset proportions and subset relative rates
  ...
  // function to get the vector of subset sizes
  inline Model::subset_sizes_t &Model::getSubsetSizes()
  {
    return _subset_sizes;
  }

  // function to get the vector of subset relative rates and store the result in _num_sites
  ...
  // function to resize several variables to have the length of _num_subsets
  ...
  // function to set the number of among-site rate categories
  ...
  // function to set the variance of among-site rates
  ...
  // function to set the proportion of invariable sites
  ...
  // function to set the exchangeabilities for a partivular subset
  ...
  // function allowing for telling the model which tree is to be used to compute the likelihood
  ...
  // function to determine which tree index is currently held by the model
  inline unsigned Model::getTreeIndex() const
  {
    return _tree_index;
  }

  // function to return whether the tree is considerd fixed
  inline bool Model::isFixedTree() const
  {
    return _tree_fixed;
  }

  // function to set the relative rates for each subset (supplied by the user)
  ...
  // function to activate and inactivate member functions
  ...
  // function that provides eigenvalues, eigenvectors, and inverse eigenvectors
  // to BeagleLib for use in computing transition probabilities
  ...
  // function providing state frequencies to BeagleLib for use in computing transition probabilities
  ...
  // function to provide the rates component of the among-site rate heterogeneity submodel to BeagleLib
  ...
  // function that sends the vector of category weights to BeagleLib
