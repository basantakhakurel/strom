...
namespace strom
{
  ...
  {
    // std::cout << "Constructing an Updater" << std::endl;
    clear();
  }
  ...
  inline Updater::~Updater()
  {
    // std::cout << "Destroying an Updater" << std::endl;
  }
  ...
  // function that provides a way to initialize values that needed to be set to zero
  ...
  // function to set heating power for different mcmc chains
  ...
  // function to tell the updater the desired acceptance rate
  inline void Updater::setTargetAcceptanceRate(double target)
  {
    _target_acceptance = target;
  }

  // function to set the prior parameters
  ...
  // function to compute a new value for the probability of the proposal
  ...
  // function to return the current value of _prob
  inline double Updater::getProb() const
  {
    return _prob;
  }

  // function to return the current value of _weight
  ...
  inline double Updater::update(double prev_lnL)
  {
    ...
    // Use alternative partials and transition probabiltiy buffer for any selected nodes
  ...
  // function to calculate the prior of the topology
  ...
  inline double Updater::calcLogEdgeLengthPrior() const
  {
    ...
    if (c != 1.0)
    {
      for (auto nd : tree->_preorder)
      {
        ...
        log_edge_length_proportions_prior += (c - 1.0) * std::log(edge_length_proportion);
      }
